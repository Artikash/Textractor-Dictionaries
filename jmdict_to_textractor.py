...
def inflect(term, depth = int(argv[4])):
	...
	for word, part_of_speech, rule in product(term.words, term.parts_of_speech, rules):
		if rule.from_part_of_speech in part_of_speech and word.endswith(rule.uninflected):
			inflections |= inflect(
				Term([word[:-len(rule.uninflected)] + rule.inflected], [rule.to_part_of_speech]), depth - 1)
	return inflections
